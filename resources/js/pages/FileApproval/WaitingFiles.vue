  ...
  <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Files Waiting for Registration</h1>
        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
          {{ files.total }} files waiting
        </span>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-md">
        ...
        <div v-if="files.data.length === 0" class="text-center py-12">
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">No files waiting</h3>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">All files have been processed.</p>
        </div>
...
const approveFile = async (fileId) => {
  ...
  try {
    await router.post(`/approval/files/${fileId}/approve-registrator`)
