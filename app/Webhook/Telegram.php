...
class Telegram extends WebhookHandler
{
    ...
    public function help()
    {
        $helpText = "🤖 <b>Mavjud buyruqlar:</b>\n\n" .
            "/start - Ro'yxatdan o'tish yoki botni ishga tushirish\n" .
            "/upload - Admin ko'rib chiqishi uchun fayl yuklash\n" .
            "/files - Yuklangan fayllar holati\n" .
            "/help - Ushbu yordam xabarini ko'rsatish\n\n" .
            "<b>📤 Yuklash jarayoni:</b>\n" .
            "1. /upload buyrug'ini ishlating\n" .
        ...
        $this->chat->message($helpText)->html()->send();
    }
    ...
    public function files()
    {
        ...
        $message .= "📤 <b>Yangi fayl yuklash:</b> /upload";

        $this->chat->message($message)->html()->send();
    }
    ...
    public function cancel()
    {
        ...
        if (Cache::get('telegram_upload_state_' . $user->id())) {
            ...
            $this->chat->message("<b>❌ Bekor qilindi</b>\n\nFayl yuklash bekor qilindi. Istalgan vaqtda /upload buyrug'ini qayta ishlatishingiz mumkin.")
                ->html()
                ->send();
            return;
        }

        $this->chat->message("<b>ℹ️ Ma'lumot</b>\n\nBekor qilinadigan faol yuklash yo'q.")
            ->html()
            ->send();
    }
    ...
    protected function handleUploadName(): void
    {
        ...
        if (strtolower(trim($fileName)) === '/cancel') {
            $this->cancel();
            return;
        }

        if (empty(trim($fileName))) {
            $this->chat->message("<b>⚠️ Xatolik!</b>\n\nIltimos, faylingiz uchun to'g'ri nom kiriting yoki bekor qilish uchun /cancel ni bosing.")
                ->html()
                ->send();
            return;
        }
    ...
    protected function handleUploadFile(): void
    {
        ...
        if ($this->message->text()) {
            ...
            $this->chat->message("<b>⚠️ Xatolik!</b>\n\nIltimos, to'g'ri fayl yuklang (hujjat, rasm, video, audio va boshqalar) yoki bekor qilish uchun /cancel ni bosing.")
                ->html()
                ->send();
            return;
        }
